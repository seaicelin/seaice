...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if(client_parser.chunked) {
        ...
        do {
            ...
            SEAICE_LOG_DEBUG(logger) << "content length = " << client_parser.content_len;
            if(client_parser.content_len + 2 <= len) {
                body.append(data, client_parser.content_len+2);
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string& url
    ...
    Uri::ptr uri = Uri::Create(url);
    if(!uri) {
        SEAICE_LOG_ERROR(logger) << "invalid url= " << url;
    }
    return std::make_shared<HttpConnectionPool>(uri->getHost()
                                          , vhost
                                          , uri->getPort()
                                          , max_size
                                          , max_alive_time
...
HttpConnectionPool::~HttpConnectionPool() {
    SEAICE_LOG_DEBUG(logger) << "~HttpConnectionPool";
}

HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = seaice::utils::getCurrentMs();
    ...
    if(!ptr) {
        ...
        if(!addr) {
            SEAICE_LOG_ERROR(logger) << "get addr failed host = " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock) {
            SEAICE_LOG_ERROR(logger) << "create socket failed addr = " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)) {
            SEAICE_LOG_ERROR(logger) << "connect socket failed addr = " << *addr;
            return nullptr;
        }
...
HttpResult::ptr HttpConnectionPool::doGet(const Uri::ptr uri
                      , uint64_t timeout_ms
                      , const std::map<std::string, std::string>& headers
                      , const std::string& body) {
    std::stringstream ss;
...
HttpResult::ptr HttpConnectionPool::doPost(const Uri::ptr uri
                      , uint64_t timeout_ms
                      , const std::map<std::string, std::string>& headers
                      , const std::string& body) {
    std::stringstream ss;
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected()
        || (ptr->m_createTime + pool->m_maxAliveTime) >= seaice::utils::getCurrentMs()
