...
Address::ptr Uri::createAddress() const {
    ...
    /*
        if (typeid(addr) == typeid(IPv6Address::ptr)) {
            IPv6Address::ptr addr6 = std::dynamic_pointer_cast<IPv6Address>(addr);
            addr6->setPort(getPort());
            //((IPv6Address*)(addr.get()))->setPort(getPort());
        } else if (typeid(addr) == typeid(IPv4Address::ptr)) {
            //((IPv4Address*)(addr.get()))->setPort(getPort());
            IPv4Address::ptr addr4 = std::dynamic_pointer_cast<IPv4Address>(addr);
            addr4->setPort(getPort());
        }
    */
    }
    return addr;
