#ifndef __SEAICE_ADDRESS_H__
#define __SEAICE_ADDRESS_H__

#include <arpa/inet.h>
#include <iostream>
#include <memory>
#include <map>
#include <stdint.h>
#include <string>
#include <sys/socket.h>
#include <sys/types.h>
#include <sys/un.h>
#include <unistd.h>
#include <vector>

namespace seaice {

class IPAddress;

class Address {
public:
    typedef std::shared_ptr<Address> ptr;
    //Address();
    virtual ~Address();
    static Address::ptr Create(const sockaddr* addr, socklen_t len);
    static bool Lookup(std::vector<Address::ptr>& vec, std::string host,
            int family = AF_INET, int type = 0, int protocol = 0);
    static Address::ptr LookupAny(std::string host, int family = AF_INET, int type = 0,
            int protocol = 0);
    static std::shared_ptr<IPAddress> LookupAnyIPAddress(std::string host, 
            int family = AF_INET, int type = 0, int protocol = 0);
    static bool GetInterfaceAddress(std::multimap<std::string, std::pair<Address::ptr, uint32_t> >& results, 
            int family = AF_INET);
    static bool GetInterfaceAddress(std::vector<std::pair<Address::ptr, uint32_t> >& results, 
            std::string iface, int family = AF_INET);
    int getFamily() const;
    std::string toString() const;
    virtual sockaddr* getAddr() = 0;
    virtual const sockaddr* getAddr() const = 0;
    virtual socklen_t getAddrLen() const = 0;
    virtual std::ostream& insert(std::ostream& os) const = 0;
    virtual bool operator<(const Address&) const;
    virtual bool operator==(const Address&) const;
    virtual bool operator!=(const Address&) const;
};

class IPAddress : public Address {
public:
    typedef std::shared_ptr<IPAddress> ptr;
    IPAddress();
    virtual ~IPAddress();
    static IPAddress::ptr Create(const char* address, uint16_t port = 0);
    virtual IPAddress::ptr broadcastAddress(uint32_t prefix_len) = 0;
    virtual IPAddress::ptr networkAddress(uint32_t prefix_len) = 0;
    virtual IPAddress::ptr subnetMask(uint32_t prefix_len) = 0;
    virtual uint32_t getPort() = 0;
    virtual void setPort(uint16_t port) = 0;
};

class IPv4Address : public IPAddress{
public:
    typedef std::shared_ptr<IPv4Address> ptr;
    static IPv4Address::ptr Create(const char* address, uint16_t port);
    IPv4Address();
    IPv4Address(const sockaddr_in& address);
    IPv4Address(uint32_t address, uint32_t port);
    ~IPv4Address();
    sockaddr* getAddr();
    const sockaddr* getAddr() const;
    socklen_t getAddrLen() const;
    std::ostream& insert(std::ostream& os) const override;
    IPAddress::ptr broadcastAddress(uint32_t prefix_len) override;
    IPAddress::ptr networkAddress(uint32_t prefix_len) override;
    IPAddress::ptr subnetMask(uint32_t prefix_len) override;
    uint32_t getPort();
    void setPort(uint16_t);

private:
    sockaddr_in m_addr;
};

class IPv6Address : public IPAddress{
public:
    typedef std::shared_ptr<IPv6Address> ptr;
    static IPv6Address::ptr Create(const char* address, uint16_t port);
    IPv6Address();
    IPv6Address(const sockaddr_in6& address);
    IPv6Address(uint8_t address[16], uint16_t port);
    ~IPv6Address();
    sockaddr* getAddr();
    const sockaddr* getAddr() const;
    socklen_t getAddrLen() const;
    std::ostream& insert(std::ostream& os) const;
    IPAddress::ptr broadcastAddress(uint32_t prefix_len) override;
    IPAddress::ptr networkAddress(uint32_t prefix_len) override;
    IPAddress::ptr subnetMask(uint32_t prefix_len) override;
    uint32_t getPort();
    void setPort(uint16_t);

private:
    sockaddr_in6 m_addr;
};

class UnixAddress : public Address {
public:
    typedef std::shared_ptr<UnixAddress> ptr;
    UnixAddress(const std::string& path);
    UnixAddress();
    ~UnixAddress();
    sockaddr* getAddr();
    const sockaddr* getAddr() const;
    socklen_t getAddrLen() const;
    std::ostream& insert(std::ostream& os) const;
    std::string getPath();
    void setAddrLen(uint32_t);
private:
    sockaddr_un m_addr;
    socklen_t m_length;
};

class UnknowAddress : public Address{
public:
    typedef std::shared_ptr<UnknowAddress> ptr;
    UnknowAddress();
    UnknowAddress(int family);
    UnknowAddress(const sockaddr& addr);
    ~UnknowAddress();
    sockaddr* getAddr();
    const sockaddr* getAddr() const;
    socklen_t getAddrLen() const;
    std::ostream& insert(std::ostream& os) const;
private:
    sockaddr m_addr;
};

std::ostream& operator<<(std::ostream& os, const Address& addr);

}
#endif